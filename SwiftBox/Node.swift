...
public struct Edges {
  ...
  fileprivate var asTuple: (Float, Float, Float, Float, Float, Float) {
...
public struct Node {
  ...
  fileprivate func createUnderlyingNode() -> NodeImpl {
    ...
    node.node.pointee.style.minDimensions = (Float(minSize.width), Float(minSize.height))
    node.node.pointee.style.maxDimensions = (Float(maxSize.width), Float(maxSize.height))
